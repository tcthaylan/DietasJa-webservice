...
{   
    ...
    public function new_record() {
		...
		if($method == 'POST') {
            ...
            && !empty($data['height']) && !empty($data['weight']) && !empty($data['sexo']) 
            && !empty($data['data_nascimento']) && !empty($data['desejo'])) {

				if(filter_var($data['email'], FILTER_VALIDATE_EMAIL)) {
					$users = new Users();
                    if($users->create($data['name'], $data['email'], $data['pass'], 
                        $data['height'], $data['weight'],  $data['sexo'], 
                        $data['data_nascimento'], $data['desejo'])) {
